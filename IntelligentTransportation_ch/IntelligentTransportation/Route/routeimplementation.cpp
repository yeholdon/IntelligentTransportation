#include "routeimplementation.h"
#include<QTime>

RouteImplementation* RouteImplementation::routeImplementation = NULL;

RouteImplementation::RouteImplementation()
{
//    生成jamLevel
...
RouteImplementation *RouteImplementation::getRoutePtr()
{
    if(routeImplementation == NULL)
    {
        routeImplementation = new RouteImplementation;
    }
    return routeImplementation;
}

//得到路径数组
QVector<int> RouteImplementation::getEdgeArray(int startPoint, int endPoint, int *jamLevel)
{
    QVector<int> edgeVec;
...
//由路径数组得到旋转数组
QVector<int> RouteImplementation::getRotateArray(QVector<int> edgeVec)
{
    ...
    {
        int edge = *iter;
        int rotate = set.value("RouteImplementationGroup/"+QString::number(edge)).toInt();
...
QString RouteImplementation::getCarControlInstruction(QVector<int> rotateVec)
{
    QVector<int>::iterator iter;
    QString cmd;  //小车控制指令
    int i= 0;
...
//设置旋转角配置文件，只调用一次
void RouteImplementation::saveSettings()
{
    QSettings set("rotateconfig.ini",QSettings::IniFormat);
    set.beginGroup("RouteImplementationGroup");
